# Running SEOFlowX on Postgres locally

SEOFlowX runs on **Cloudflare D1 (SQLite) by default**. Postgres is an opt-in
...
```sh
docker run --name seoflowx-postgres \
  -e POSTGRES_USER=seoflowx \
  -e POSTGRES_PASSWORD=seoflowx \
  -e POSTGRES_DB=seoflowx \
  -p 5433:5432 \
  -d postgres:16
```
...
```sh
docker exec seoflowx-postgres pg_isready -U seoflowx -d seoflowx
```

The connection string is:

```
postgres://seoflowx:seoflowx@localhost:5433/seoflowx
```
...
```sh
POSTGRES_DATABASE_URL=postgres://seoflowx:seoflowx@localhost:5433/seoflowx \
  pnpm db:migrate:pg
```
...
docker exec seoflowx-postgres psql -U seoflowx -d seoflowx -c "\dt"

# Inspect rows the app writes (e.g. after creating a project / saving keywords)
docker exec seoflowx-postgres psql -U seoflowx -d seoflowx -c "select count(*) from projects;"
```
