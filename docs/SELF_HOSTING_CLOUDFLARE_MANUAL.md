...
Use this flow if the [Deploy to Cloudflare button](./SELF_HOSTING_CLOUDFLARE.md) fails with `Cannot provision a KV Namespace with the title "seoflowx" because it already exists`. The reliable path is to create Cloudflare resources yourself, put their IDs into `wrangler.jsonc`, then deploy with Wrangler.

## 1) Clone your SEOFlowX repo

Fork `Hichamtach/SEOFlowX` on GitHub if you want a repo you control for future updates, then clone it locally:

```bash
git clone https://github.com/YOUR_GITHUB_USER/seoflowx.git
cd seoflowx
corepack enable
pnpm install
```
...
```bash
pnpm exec wrangler kv namespace create seoflowx-YOUR_SUFFIX
pnpm exec wrangler kv namespace create seoflowx-oauth-YOUR_SUFFIX
pnpm exec wrangler d1 create seoflowx-YOUR_SUFFIX
pnpm exec wrangler r2 bucket create seoflowx-YOUR_SUFFIX
```
...
"d1_databases": [
  {
    "binding": "DB",
    "database_name": "seoflowx-YOUR_SUFFIX",
...
"r2_buckets": [
  {
    "bucket_name": "seoflowx-YOUR_SUFFIX",
    "binding": "R2",
  },
],
```
...
```bash
pnpm exec wrangler r2 bucket lifecycle add seoflowx-YOUR_SUFFIX dataforseo-cache-expiry dataforseo-cache/ --expire-days 7
```
...
See [Operations](./SELF_HOSTING_CLOUDFLARE_OPERATIONS.md) for connecting MCP clients, updating to the latest SEOFlowX version, and giving teammates access.
