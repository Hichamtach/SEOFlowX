...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database seoflowx
   ```

2. If the output looks right, apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database seoflowx --apply --confirm-remote-apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database seoflowx --validate-only
   ```
...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database seoflowx --local
   ```

2. Apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database seoflowx --local --apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database seoflowx --local --validate-only
   ```
