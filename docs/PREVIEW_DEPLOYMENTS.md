...
SEOFlowX preview stages use isolated Cloudflare resources and a shared,
Alchemy-managed Cloudflare Access boundary.

## Security model

- Preview Workers are named `seoflowx-<stage>` and served from
  `seoflowx-<stage>.<WORKERS_SUBDOMAIN>`.
- One persistent Access application protects
  `seoflowx-*.<WORKERS_SUBDOMAIN>` before any preview Worker exists.
- Production uses the unsuffixed `seoflowx` Worker on `app.seoflowx.com` and
  `www.app.seoflowx.com`. It does not match the preview wildcard and is not
...
```text
seoflowx-*.your-subdomain.workers.dev
```
...
`https://seoflowx-<stage>.<WORKERS_SUBDOMAIN>` — hosted previews use that as
...
External (Hichamtach/SEOFlowX) PRs never deploy from CI — fork code must not
...
```sh
git fetch https://github.com/Hichamtach/SEOFlowX.git pull/<pr>/head
git worktree add --detach ../seoflowx-pub-<pr> FETCH_HEAD
cp .env.preview ../seoflowx-pub-<pr>/
(cd ../seoflowx-pub-<pr> && pnpm install --frozen-lockfile && pnpm exec vite build --mode preview)
rm -rf dist && cp -R ../seoflowx-pub-<pr>/dist dist
git worktree remove --force ../seoflowx-pub-<pr>
...
1. Append `--dry-run` to the alchemy command and read the plan — every prod
   resource (D1 `seoflowx`, KV `every-super-seo`/`OAUTH_KV`, R2 `seoflowx`,
   Hyperdrive `seoflowx`, Worker `seoflowx`) should be adopted, none created.
2. Diff `.env.production` against the live worker's secrets
   (`GET /accounts/:id/workers/scripts/seoflowx/secrets`): alchemy's deploy
...
6. Note: prod already serves on `seoflowx.<subdomain>.workers.dev` (alchemy
