---
title: "Set up SEOFlowX MCP"
description: "Connect SEOFlowX MCP to Claude, Codex, and other AI clients."
---

SEOFlowX MCP lets compatible AI clients call SEOFlowX tools for keyword research, SERP inspection, local business research, competitive search intelligence, domain research, backlink overview, saved keywords, rank tracking, and Google Search Console performance and URL inspection.
...
The first connection sends you through SEOFlowX login. After authorization, your MCP client can call SEOFlowX tools with the project context and account scopes you approved.

For the most current setup UI and a copyable endpoint, open [AI & MCP in SEOFlowX](https://app.seoflowx.com/ai).

## Claude Code

Use user scope to make SEOFlowX available across projects. Use local scope for the current repository.

```bash
claude mcp add --transport http --scope user seoflowx https://app.seoflowx.com/mcp
```

After adding the server, approve the SEOFlowX login when prompted.

## Claude Desktop

1. Open Settings -> Connectors.
2. Click Add custom connector.
3. Paste `https://app.seoflowx.com/mcp`.
4. Approve the SEOFlowX login when prompted.
...
```json
{
  "mcpServers": {
    "seoflowx": {
      "url": "https://app.seoflowx.com/mcp"
    }
  }
}
```
...
SEOFlowX MCP exposes tools for SEO research workflows:
...
- List saved keywords from an SEOFlowX project.
- Save useful keywords back to SEOFlowX.
...
Once SEOFlowX MCP is connected, [set up SEOFlowX Agent Skills](/docs/skills/setup). MCP gives your agent access to SEOFlowX data. Skills are separate `SKILL.md` files that tell your agent how to use that data for specific SEO jobs.
...
If your client cannot connect, check that the server URL is exactly `https://app.seoflowx.com/mcp`.

If authorization fails, disconnect the SEOFlowX server in your client, add it again, and repeat the login flow.

If your agent cannot find a project, ask it to list SEOFlowX projects first and use the returned project ID in later tool calls.
