...
description: "Deploy SEOFlowX to your own Cloudflare account for internet-facing, multi-device, or team use."
---

Host SEOFlowX on Cloudflare for internet-facing self-hosting across multiple devices or with your team. It works on Cloudflare's free plan.

## 1) Deploy from GitHub

[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/Hichamtach/SEOFlowX)
...
If deploy fails with `Cannot provision a KV Namespace with the title "seoflowx" because it already exists`, use the [manual deploy with Wrangler guide on GitHub](https://github.com/Hichamtach/SEOFlowX/blob/main/docs/SELF_HOSTING_CLOUDFLARE_MANUAL.md) instead.

## 2) Configure authentication and secrets

In the Cloudflare dashboard:

1. Go to `Compute` -> `Workers & Pages` -> your SEOFlowX Worker.
2. Open `Settings`.
...
```bash
npx wrangler r2 bucket lifecycle add seoflowx dataforseo-cache-expiry dataforseo-cache/ --expire-days 7
```

If you changed the R2 bucket name during deploy, replace `seoflowx` with your bucket name.
...
3. SEOFlowX should load after login.

If login fails, re-check the three secrets and Access toggle.

## Connect the MCP server through Cloudflare Access

Use the same Cloudflare Access application that protects your SEOFlowX Worker. Managed OAuth is required for MCP clients and is not enabled by default.

1. Open Cloudflare Zero Trust.
2. Go to `Access controls` -> `Applications`.
3. Find your SEOFlowX application, then select `Edit`.
...
## Give teammates access to SEOFlowX

1. Open Cloudflare Zero Trust.
2. Go to Access -> Applications.
3. Open your SEOFlowX application.
4. Edit the `Allow` policy.
5. Add teammate emails (or your company email domain / group).
6. Save.

After saving, teammates can open your SEOFlowX URL and sign in through Cloudflare Access. SEOFlowX will use a shared workspace for everyone allowed by the policy.

## Advanced guides on GitHub

- [Manual deploy with Wrangler](https://github.com/Hichamtach/SEOFlowX/blob/main/docs/SELF_HOSTING_CLOUDFLARE_MANUAL.md): create the Cloudflare resources yourself and deploy with the CLI.
- [Operations](https://github.com/Hichamtach/SEOFlowX/blob/main/docs/SELF_HOSTING_CLOUDFLARE_OPERATIONS.md): update to the latest SEOFlowX version and manage telemetry.
