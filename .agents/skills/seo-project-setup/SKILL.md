...
### 5. Verify SEOFlowX MCP

After the user has described the company, website, goals, and positioning, check that SEOFlowX MCP is configured and mapped to the right project:
...
5. If the MCP is unavailable, tell the user to connect SEOFlowX MCP before continuing with live SEOFlowX data.
...
After intake, recommend one next SEOFlowX workflow:
