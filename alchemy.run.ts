...
// - Stage "hosted-prod": names the EXISTING seoflowx.com production resources
...
export default Alchemy.Stack(
  ...
  Effect.gen(function* () {
    ...
    if (prod) {
      ...
      if (!authUrl) {
        return yield* Effect.die(
          new Error(
            "Set BETTER_AUTH_URL (https://app.seoflowx.com) in .env.production.",
          ),
        );
      }
    ...
    const app = yield* Cloudflare.Worker("open-seo", {
      ...
      domain: prod ? ["app.seoflowx.com", "www.app.seoflowx.com"] : undefined,
      ...
      // Prod adopts the live worker serving app.seoflowx.com; never delete it
