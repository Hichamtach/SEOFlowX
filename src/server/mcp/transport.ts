...
import { registerSeoFlowXMcpTools } from "@/server/mcp/server";

function createSeoFlowXMcpServer() {
  const server = new McpServer(
    {
      ...
      websiteUrl: "https://seoflowx.com",
      icons: [
        {
          src: "https://seoflowx.com/android-chrome-512x512.png",
          mimeType: "image/png",
          sizes: ["512x512"],
        },
      ],
    ...
    {
      instructions:
        "SEOFlowX research tools use credits. Proceed with normal focused research, but ask the user for confirmation before planned batches over 2,000 credits.",
    },
  );
  registerSeoFlowXMcpTools(server);

  return server;
}

export async function handleAuthenticatedSeoFlowXMcpRequest(
  request: Request,
  props: unknown,
  env: unknown,
  ...
  return handleSeoFlowXMcpRequest(request, result.data, env, ctx);
}

export async function handleSelfHostedSeoFlowXMcpRequest(
  request: Request,
  ...
  if (request.method === "OPTIONS") {
    return handleSeoFlowXMcpRequest(request, undefined, env, ctx);
  }
  ...
  return handleSeoFlowXMcpRequest(request, props, env, ctx);
}

function handleSeoFlowXMcpRequest(
  request: Request,
  ...
  const server = createSeoFlowXMcpServer();
