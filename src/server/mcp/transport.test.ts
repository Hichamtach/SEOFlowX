...
vi.mock("@/server/mcp/server", () => ({
  registerSeoFlowXMcpTools: vi.fn(),
}));
...
function createMcpRequest() {
  return new Request("https://seoflowx.test/mcp", {
    method: "POST",
    headers: {
...
describe("handleSelfHostedSeoFlowXMcpRequest", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("accepts local no-auth MCP requests with the local admin context", async () => {
    const { handleSelfHostedSeoFlowXMcpRequest } =
      await import("@/server/mcp/transport");

    const response = await handleSelfHostedSeoFlowXMcpRequest(
      createMcpRequest(),
      "local_noauth",
      {},
      ctx,
    ...
    expect(
      ...
      clientId: null,
      scopes: [],
      audience: "https://seoflowx.test/mcp",
      subject: "local-admin",
      baseUrl: "https://seoflowx.test",
    });
  });
  ...
  it("returns 405 for the standalone GET SSE stream without building a server", async () => {
    ...
    const response = await handleSelfHostedSeoFlowXMcpRequest(
      new Request("https://seoflowx.test/mcp", {
        method: "GET",
  ...
  it("lets the MCP transport handle OPTIONS without auth context", async () => {
    ...
    const response = await handleSelfHostedSeoFlowXMcpRequest(
      new Request("https://seoflowx.test/mcp", { method: "OPTIONS" }),
      "cloudflare_access",
      {},
      ctx,
    );
  ...
  it("serves directory metadata in the initialize response", async () => {
    ...
    await handleSelfHostedSeoFlowXMcpRequest(
      createMcpRequest(),
      "local_noauth",
      {},
      ctx,
