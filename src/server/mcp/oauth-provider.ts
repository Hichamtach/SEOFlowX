...
import { handleAuthenticatedSeoFlowXMcpRequest } from "@/server/mcp/transport";
...
export type SeoFlowXOAuthEnv = Env & {
...
function getOAuthHelpers(env: SeoFlowXOAuthEnv) {
...
function createDefaultHandler(
  appFetch: AppFetch,
): ExportedHandlerWithFetch<SeoFlowXOAuthEnv> {
  return {
...
const mcpApiHandler: ExportedHandlerWithFetch<SeoFlowXOAuthEnv> = {
  async fetch(request, env, ctx) {
    return handleAuthenticatedSeoFlowXMcpRequest(
      request,
...
export function createSeoFlowXOAuthProvider(appFetch: AppFetch) {
  const options: OAuthProviderOptions<SeoFlowXOAuthEnv> = {
    apiRoute: MCP_ROUTE,
